...
public class WorkDetailsController {
    ...
    public ResponseEntity<List<WorkDetails>> getByEmployee(@PathVariable Long id) {
        return ResponseEntity.ok(workDetailsService.getByEmployee(id));
    }

    @GetMapping("/manager/{id}")
    public ResponseEntity<List<WorkDetails>> getByManager(@PathVariable Long id) {
        return ResponseEntity.ok(workDetailsService.getByManager(id));
    }

    @GetMapping("/project/{id}")
    public ResponseEntity<List<WorkDetails>> getByProject(@PathVariable Long id) {
        return ResponseEntity.ok(workDetailsService.getByProject(id));
    }

    @GetMapping("/activity/{id}")
    public ResponseEntity<List<WorkDetails>> getByActivity(@PathVariable Long id) {
        return ResponseEntity.ok(workDetailsService.getByActivity(id));
    }

    @GetMapping("/employee/{empId}/project/{projId}")
    public ResponseEntity<List<WorkDetails>> getByEmployeeAndProject(@PathVariable Long empId, @PathVariable Long projId) {
        return ResponseEntity.ok(workDetailsService.getByEmployeeAndProject(empId, projId));
    }

    @GetMapping("/manager/{mgrId}/project/{projId}")
    public ResponseEntity<List<WorkDetails>> getByManagerAndProject(@PathVariable Long mgrId, @PathVariable Long projId) {
